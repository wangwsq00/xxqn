...
第一批国漫立绘，替换战斗 / 洞府 / 灵宠色块。不改玩法数值。文件名**不要**用 rabbit / wolf。
...
UI 切图为次要资源，本批未接入（无合适 uploads）：

| 文件 | 说明 |
|------|------|
| `public/assets/ui/ui_panel.png` | 可选面板九宫；有图再做轻量 Hub/Pet 换皮 |
| `public/assets/ui/ui_btn.png` | 可选按钮；同上 |
...
- `src/scenes/HubScene.ts` / `PetScene.ts` / `TrialSelectScene.ts`：头像与缩略图
